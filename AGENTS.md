...
- **Assets ship pre-compressed**: `mise run upload-assets` stores text assets brotli-compressed at quality 11 with `Content-Encoding: br`. A CDN compresses at a fixed low quality it does not let you raise (measured: brotli q4, worse than its own gzip), so quality 11 is only reachable by compressing at build time. Clients that cannot take brotli are unaffected — the edge decompresses and re-encodes for them. Existing objects are content-hashed and skipped, so `--force` is what makes stored assets pick up a compression change; `--no-compress` opts out.
