...
- **TypeScript dev scripts run through Vite, never tsx**: `src/` needs the SWC Lingui transform and Vite's `define` globals, so `packages/core/dev/scripts/*.ts` run as `node dev/run-script.mjs <script> [args]`. A script exports its entry point as default and does no work when imported. `src/__tests__/dev-scripts.test.ts` loads every script and fails on any task that goes around the runner.
