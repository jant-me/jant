...
想要一个成员跟着规则走、而不是靠你手工归类的合集，见[智能合集](#智能合集)。
...
智能合集是一种成员由条件决定的合集。条件只写一次——「所有 Quote」「2024 年带图的 Note」——符合的帖子自动在里面，以后写的新帖子也一样。

对读者来说它就是一个合集：地址在 `/{slug}`，有标题、描述、帖子列表和自己的 feed。数字下面多一行说明它靠什么收录，那一行可以点，落到显示同一批帖子的归档页，想再筛的人有地方去。

从 **Collections → ⋯ → 新建智能合集** 创建。弹窗会随着你改条件实时数出命中多少，存之前就能看见这条条件做了什么。

条件之间是「且」——帖子要同时满足全部条件。每个条件最多出现一次：

| 条件         | 选出                               |
| ------------ | ---------------------------------- |
| `Collection` | 属于某个合集的 Thread              |
| `Format`     | Note、Link 或 Quote                |
| `Title`      | 有标题或没标题的帖子               |
| `Year`       | 某一年发布的帖子                   |
| `Media`      | 带任意附件、不带附件，或带某类附件 |
| `Replies`    | 有回复的 Thread，或没有回复的单帖  |
| `Visibility` | 公开、精选，或不在 Latest 里显示   |

一条条件都不写也能存，那就是「收录全部帖子」。

### 智能合集做不到的事

成员由条件决定，所以合集能做的有几件事它没有对应物：
...
因为第一行，撰写帖子时的合集选择器里没有智能合集。那里没有什么可选的：帖子靠符合条件进去，不靠被加进去。

### 永远是公开的

没有私密的智能合集。`Visibility` 只能取「公开」「精选」「不在 Latest 里显示」，所以智能合集的页面不会对访客回 404。

这和私密帖子会不会漏出来是两件事。每个页面用的是和合集页完全一样的逐帖规则：访客只看得到本来就看得到的东西，你登录之后看到的条数可能更多。

想给自己留一个私密的筛选视图，就去 `/archive` 筛完收藏那个地址——它本来就能用，你失去的只是给它起名字。
...
除了默认 slug，帖子和 Collection 都可以设置自定义路径，也可以配置跳转规则。这些都在 **设置 → 高级 → 自定义 URL** 里管理（路径 `/settings/custom-urls`）。
...
用已退役的**归档**类型建的路径继续能用，见[自定义归档视图（已退役）](#自定义归档视图已退役)。
...
自定义 URL 原来有一种「归档」类型：一个短路径，存着手打的归档查询串。[智能合集](#智能合集)取代了它——结果一样，但不用把 `format=note&title=none` 敲进输入框，还多了标题、描述和 feed。

已有的归档地址继续能用，也没有让它们失效的打算。要换成智能合集，先记下这条记录上显示的查询串，例如 `/archive?format=note&title=none`，然后删除这个自定义 URL，再在同一路径用同样的条件新建智能合集。必须先删除，因为自定义 URL 存在期间一直占着这个地址，别的东西存不进去。筛选「私密」的路径没有对应的智能合集，因为智能合集总是公开的，这种路径保持原样即可。
