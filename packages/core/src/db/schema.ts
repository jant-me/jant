 ...
 *
 * Kept apart from `collection` rather than folded into it. The two share an
 * identity shape — a path registry row, a title, a description — but half the
 * columns of a merged table would be meaningless for half its rows, every query
 * would branch, and a smart collection can filter *by* a collection, which in
 * one table becomes a self-reference and a cycle to defend against. The
 * resemblance belongs in a read model, not in the storage.
 *
 * The condition columns are declared once in `lib/filter-dimensions.ts`; this
 * table is where they land. `NULL` always means "this dimension is not part of
 * the conditions".
 *
 * Never private: `visibility` is constrained to the public archive values, so
...
export const smartCollections = sqliteTable(
  "smart_collection",
  {
    ...
    hasReplies: integer("has_replies", { mode: "boolean" }),
